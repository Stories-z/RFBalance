...
def run(args):
    ...
    for i in range(len(task_names)):
        ...
        if i > 0:
            o_model = copy.deepcopy(agent.model.state_dict())

            agent.model_optimizer.switch = True
            agent.train_task(train_loader, val_loader)
            p_model = copy.deepcopy(agent.model.state_dict())

            agent.model.load_state_dict(o_model)
            agent.init_model_optimizer()
            agent.model_optimizer.switch = False
            agent.train_task(train_loader, val_loader)
            q_model = agent.model.state_dict()

            ans = o_model

            alpha = 1.0 /  (i + 1)
            for k in o_model.keys():
                ans[k] =  p_model[k] * (1 - alpha) + q_model[k] * alpha

            agent.model.load_state_dict(ans)
        else:
            agent.train_task(train_loader, val_loader)
        ...
        # AfterTrain
        agent.after_train(train_loader, val_loader)
